...
public class EnemyAI : MonoBehaviour {
    ...
    public event Action OnEnemyDeath;  // Событие смерти

    public void SetDeathState()
    {
        _navMeshAgent.ResetPath();
        _currentState = State.Death;
        OnEnemyDeath?.Invoke();  // Уведомляем о смерти
        Destroy(gameObject, 2f); // Уничтожаем объект через 1 секунду (можно настроить)
    }
